...
class ShowProducts extends Component {
  ...
  peticionGet = () => {
    axios
      ...
      axios
      .get('https://back-farmacia.onrender.com/api/proveedor/')
      .then((response) => {
  ...
  peticionPut = () => {
    ...
    if (form.imagen) {
      if (form.imagen instanceof File) {
        ...
        parametros.append('imagen', form.imagen); // Enviar la imagen original del producto
      }
    }
    axios
  ...
  handleChange = async (e) => {
    ...
    if (e.target.name === 'imagen') {
      ...
      if (e.target.files && e.target.files[0]) {
        ...
        // Si no hay archivos seleccionados, establece la imagen como null
        await this.setState({
          form: {
            ...form,
            imagen: null,
  ...
  .map((product) =>  {
              return (
                <tr key={product.id_producto}>
                  ...
                  <td>{product.stock}</td>
                  <td>
                    {product.imagen && <img src={`https://back-farmacia.onrender.com/uploads/${product.imagen}`} style={{ maxWidth: '100px' }} alt='Product' />}
                  </td>
                  <td>
  ...
  id='id_proveedor'
  onChange={this.handleChange}
  value={form ? form.id_proveedor : null}
>
  <option value="">
